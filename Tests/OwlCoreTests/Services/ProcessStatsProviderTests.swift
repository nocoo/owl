...
struct ProcessStatsProviderTests {
    ...
    @Test func cpuTimeFormattedHoursAndMinutes() {
        let r = ProcessRanking(
            id: "test", cpuSeconds: 3661,
            memoryMB: 100, instanceCount: 1
        )
    ...
    @Test func memoryFormattedMB() {
        let r = ProcessRanking(
            id: "test", cpuSeconds: 0,
            memoryMB: 512, instanceCount: 1
        )
    ...
    @Test func processRankingIsIdentifiable() {
        let r = ProcessRanking(
            id: "foo", cpuSeconds: 10,
            memoryMB: 50, instanceCount: 1
        )
        #expect(r.id == "foo")
    }
