...
| # | Task | Commit | Status |
|---|------|--------|--------|
| 0.1 | Create Package.swift with Owl (executable) + OwlCore (library) + OwlCoreTests targets, macOS 14.0 | | [ ] |
| 0.2 | Create directory structure: Sources/Owl/, Sources/OwlCore/, Tests/OwlCoreTests/ with placeholder files | | [ ] |
| 0.3 | Add .swiftlint.yml (strict mode) + install SwiftLint config | | [ ] |
| 0.4 | Add Git Hooks: scripts/pre-commit.sh (UT + Lint), scripts/pre-push.sh (Integration), .githooks setup | | [ ] |
| 0.5 | Update .gitignore for SPM (.build/, .swiftpm/) | | [ ] |
| 0.6 | Verify: `swift build` + `swift test` + `swiftlint` all pass | | [ ] |
...
| # | Task | Commit | Status |
|---|------|--------|--------|
| 1.2a.1 | Define `PatternDetector` protocol (id, isEnabled, accepts, process, tick) | | [ ] |
...
| # | Task | Commit | Status |
|---|------|--------|--------|
| 1.2b.1 | Implement `SlidingWindowCounter` (Ring Buffer time buckets, O(1) increment/advance/total) + UT | | [ ] |
| 1.2b.2 | UT: Ring Buffer boundary conditions (full window, cross-window advance, full expiry) | | [ ] |
...
| # | Task | Commit | Status |
|---|------|--------|--------|
| 1.3.1 | Create `TestFixtures.swift` with real log samples from docs for all 14 patterns | | [ ] |
| 1.3.2 | P01 ThermalPattern (ThresholdDetector config + regex) + UT with real log | | [ ] |
| 1.3.3 | P02 CrashLoopPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.4 | P03 DiskFlushPattern (ThresholdDetector config + regex) + UT | | [ ] |
| 1.3.5 | P04 WiFiPattern (ThresholdDetector config + dual metrics) + UT | | [ ] |
| 1.3.6 | P05 SandboxPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.7 | P06 SleepAssertionPattern (StateDetector config + regex) + UT | | [ ] |
| 1.3.8 | P07 CrashSignalPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.9 | P08 BluetoothPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.10 | P09 TCCPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.11 | P10 JetsamPattern (Threshold + Rate hybrid) + UT | | [ ] |
| 1.3.12 | P11 AppHangPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.13 | P12 NetworkPattern (RateDetector global aggregate) + UT | | [ ] |
| 1.3.14 | P13 USBPattern (RateDetector config + regex) + UT | | [ ] |
| 1.3.15 | P14 DarkWakePattern (RateDetector global aggregate) + UT | | [ ] |
